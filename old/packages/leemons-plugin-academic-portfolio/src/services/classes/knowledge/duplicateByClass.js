...
async function duplicateByClass(
  ...
  return global.utils.withTransaction(
    async (transacting) => {
      ...
      await leemons.events.emit('before-duplicate-classes-knowledges', {
        classKnowledges,
        transacting,
      });
      ...
      const newItems = await Promise.all(
        _.map(classKnowledges, ({ id, ...item }) =>
          table.classKnowledges.create(
            {
              ...item,
              class:
      ...
      await leemons.events.emit('after-duplicate-classes-knowledges', {
        classKnowledges,
