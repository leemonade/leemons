...
async function updateSubjects(deploymentIds) {
  ...
  const course = await groups.findOne({
    type: "course",
    deploymentID: { $in: deploymentIds },
  });
  ...
  const allSubjects = await subjects
    .find({ deploymentID: { $in: deploymentIds } })
    .toArray();
...
async function removeGroupsAndClassgroups(deploymentIds) {
  ...
  const groupsDeletionResults = await groups.deleteMany({
    id: { $in: groupsToDeleteIds },
  });
...
(async () => {
  try {
    ...
    const freeDeploymentsIds =
      freeDeployments?.map((deployment) => deployment.id) ?? [];
