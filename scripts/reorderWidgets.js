...
const ZONE_ORDERS = {
  ...
  "leebrary.drawer.tabs": [
    "leebrary.drawer.tabs.library",
    "leebrary.drawer.tabs.new",
  ],
};
...
async function getProfiles(deploymentID) {
  ...
  return profiles
    .find({ deploymentID, sysName: { $in: ["student", "teacher"] } })
    .toArray();
}
...
async function updateOrderItemsInZone(items) {
  ...
  return Promise.all(
    _.map(items, (item) =>
      collection.updateOne({ id: item.id }, { $set: { order: item.order } })
    )
  );
}
...
async function updateProfileItemsInZone(items) {
  ...
  const widgetItemProfilesCollection = database.collection(
...
async function processDeploymentLoop(deploymentID, deployments) {
  ...
  const studentProfile = profiles.find(
    (profile) => profile.sysName === "student"
  );
  const teacherProfile = profiles.find(
    (profile) => profile.sysName === "teacher"
  );
