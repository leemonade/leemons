...
async function getPrograms(programIds) {
  const programsCollection = database.collection(
    "v1::academic-portfolio_programs"
  );
...
async function getAllGroupIds(classIds) {
  const classCoursesCollection = database.collection(
    "v1::academic-portfolio_classgroups"
  );
...
async function getClassDetails(calendars) {
  ...
  const classesCollection = database.collection(
    "v1::academic-portfolio_classes"
  );
  const classCalendars = await classesCollection
    .find({ id: { $in: classIds } })
    .toArray();
  ...
  const subjectsCollection = database.collection(
    "v1::academic-portfolio_subjects"
  );
  ...
  Object.keys(classes).forEach((classId) => {
    ...
    if (classes[classId].program) {
      classes[classId].program = allPrograms.find(
        (p) => p.id === classes[classId].program
      );
    }
  });
  ...
  Object.keys(classes).forEach((classId, i) => {
    ...
    if (program.maxNumberOfCourses > 1) {
      ...
      try {
        ...
        if (
          subject.course?.length &&
          (subject.course || "").startsWith("lrn")
        ) {
    ...
    console.log(
      "classObject.subject.internalId",
      classObject.subject.internalId
    );
    console.log("\n");
  });
...
async function updateCalendarNames(classes) {
  ...
  Object.keys(classes).forEach((classId) => {
    ...
    const subjectSuffix =
      classObject.subject.internalId && ` - ${classObject.subject.internalId}`;
