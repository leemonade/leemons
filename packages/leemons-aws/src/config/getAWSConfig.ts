...
export function getAWSConfig({
  config = {},
  credentials,
}: GetAWSConfigProps): AWSClientConfig {
  ...
  if (credentials?.accessKeyId) {
    (clientConfig.credentials as MutableAwsCredentials).accessKeyId =
  ...
  if (credentials?.sessionToken) {
    (clientConfig.credentials as MutableAwsCredentials).sessionToken =
