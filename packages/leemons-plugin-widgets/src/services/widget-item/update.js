...
async function update(zoneKey, key, { url, name, description, profiles, transacting } = {}) {
  ...
  if (_.isArray(profiles)) {
    await table.widgetItemProfile.deleteMany({ zoneKey, key }, { transacting });
  }
  const promises = [table.widgetItem.update({ zoneKey, key }, toUpdate, { transacting })];
