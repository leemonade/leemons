...
class MoleculerRunner {
            ...
            if (this.isDirectory(svcPath)) {
              ...
              if (files.length == 0)
                return this.broker.logger.warn(
                  kleur.yellow().bold(`There is no service files in directory: '${svcPath}'`)
                );
  ...
  _run() {
    return Promise.resolve()
      ...
      .then(() => {
        this.broker.getActionURLName = function (action) {
          return `[${action.rest.method || '*'}]${action.name.replace(`.${action.rawName}`, '')}${
            action.rest.path || action.rest
          }`.replace(/\./g, '/');
        };
        this.broker.restUrls = {};
        _.forEach(this.broker.services, (service) => {
          _.forIn(service._serviceSpecification.actions, (action) => {
            if (action.rest) {
              this.broker.restUrls[this.broker.getActionURLName(action)] = action;
            }
          });
        });
        return this.broker;
