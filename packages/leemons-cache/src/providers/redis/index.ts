...
export function getClientConfig(
  _config?: string | Redis | RedisConfig
): RedisConfig | null {
  if (!_config) {
...
function tracingWrapper<T extends (...args: any[]) => Promise<any>>(
  f: T,
  ctx?: Context
): T {
  ...
  return ({ pluginName, ctx }: CachePluginOptions) => {
    ...
    const wrappedQueries = Object.fromEntries(
      Object.entries(queriesObject).map(([key, f]) => [
        key,
        typeof f === "function"
          ? tracingWrapper(f as (...args: any[]) => Promise<any>, ctx)
          : f,
      ])
    );
