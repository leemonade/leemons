...
it("Should handle permission roles correctly when roles are not passed", () => {
  ...
  const mockPermissions = [
    {
      permissionName: ctx.prefixPN(permissionSeparator + assetIds[0]),
      actionNames: ["viewer"],
    },
    {
      permissionName: ctx.prefixPN(permissionSeparator + assetIds[1]),
      actionNames: ["owner"],
    },
    {
      permissionName: ctx.prefixPN(`${permissionSeparator}otherAsset`),
      actionNames: ["editor"],
    },
  ];
  ...
  // Act
  const response = handlePermissionsRoles({
