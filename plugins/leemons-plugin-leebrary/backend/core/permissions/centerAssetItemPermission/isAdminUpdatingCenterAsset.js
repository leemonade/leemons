...
const PROFILES_WITH_ADMIN_MANAGEABLE_ASSETS = [
  SYS_PROFILE_NAMES.CONTENT_DEVELOPER,
];
...
async function isAdminUpdatingCenterAsset({ asset, ctx }) {
  ...
  if (
    ...
    !PROFILES_WITH_ADMIN_MANAGEABLE_ASSETS.includes(
      assetOwnerUserAgent?.profile?.sysName
    )
  ) {
    return false;
  }
  ...
  return ctx.tx.call("users.permissions.existItems", {
    query: {
      permissionName: getCenterPermissionName({
        centerId: assetOwnerUserAgent.center.id,
      }),
      item: asset.id,
      actionName: "admin",
    },
