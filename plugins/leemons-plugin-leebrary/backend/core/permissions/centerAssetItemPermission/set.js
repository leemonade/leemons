...
async function determinePermissionAction({
  ...
  const hasCenterPermission = await ctx.tx.call(
    "users.permissions.existItems",
    {
      query: {
  ...
  const shouldHavePermission = await shouldAssetHavePermission({
    assetDetail,
    isPublishing,
    ctx,
  });
...
async function set({ assetId, isPublishing = false, ctx }) {
  ...
  if (ALLOWED_PROFILES.includes(userAgentInfo?.profile?.sysName)) {
    const { assetDetail, centerAssetPermissionName } =
      await getCenterPermissionData({
        assetId,
