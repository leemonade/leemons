...
it("Should not throw, instead it returns false and uses ctx.logger to log addFiles() errors if any", async () => {
  ...
  // Act
  const responseWithWrongId = await handleFiles({
    newFile: wrongFileId,
    assetId: asset.id,
    ctx,
  });
  const testWithWrongId = async () =>
    handleFiles({ newFile: wrongFileId, assetId: asset.id, ctx });

  addFiles.mockImplementation(() => {
    throw new Error("Boom!");
  });
  const responseWithError = await handleFiles({
    newFile,
    assetId: asset.id,
    ctx,
  });
