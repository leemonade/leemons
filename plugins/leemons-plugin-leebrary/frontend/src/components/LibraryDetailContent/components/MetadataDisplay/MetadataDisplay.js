...
const MetadataDisplay = ({ metadata, onCopy }) => {
  ...
  const fileSizeDocument =
    metadata?.fileType === "document" && metadata?.metadata[0]?.value;
  ...
  const variantIconLabel =
    (metadata?.variantTitle ?? metadata?.fileType ?? metadata?.variant)
      ?.charAt(0)
      ?.toUpperCase() +
    (metadata?.variantTitle ?? metadata?.fileType ?? metadata?.variant)?.slice(
      1
    );
