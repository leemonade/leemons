      ...
      asset: {
        ...
        renderHTML: ({ asset: { canAccess, ...asset } }) => {
          ...
          return {
            ...cleanAsset,
            fileId:
              asset.fileid ?? asset.fileId ?? asset.file?.id ?? asset.file,
            coverId:
              asset.coverid ??
              asset.coverId ??
    ...
    return [
      {
        ...
        getAttrs: (element) => {
          ...
          if (!isEmpty(asset)) {
            ...
            asset.coverId =
              coverIdOptions.length > 0 ? coverIdOptions[0] : undefined;

            const fileIdOptions = _.compact([
              asset.fileid,
              asset.fileId,
              asset.file?.id,
            ]).filter((id) => id !== "undefined");
            asset.fileId =
              fileIdOptions.length > 0 ? fileIdOptions[0] : undefined;
