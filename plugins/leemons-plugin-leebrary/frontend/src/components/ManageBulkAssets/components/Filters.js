...
const Filters = ({ assets, onFiltersChange, t }) => {
  ...
  useEffect(() => {
    const filteredAssets = selectedSubject
      ? assets.filter((asset) =>
          asset.subjects?.some(
            (subject) => subject.subject === selectedSubject[0]
          )
        )
      : assets;
        ...
        valueComponent={(item) => {
          return (
            <SubjectItem
              ...
              isValueComponent
              subject={
                subjectOptions.find((s) => s.value === item.value)?.subject
              }
            />
          );
        }}
