  ...
  const getCategories = async () => {
    ...
    if (classes) {
      ...
      store.subjects = _.map(store.subjects, (subject) => ({
        ...subject,
        image:
          typeof subject?.image === "string"
            ? subject.image
            : getClassImage({ subject }),
      }));
    }
  ...
  const selectCategory = useCallback(
    (key) => {
      if (
        ...
        key === CATEGORY_LEEBRARY_SHARED &&
        category?.key !== CATEGORY_LEEBRARY_SHARED
      ) {
        setCategory({ key: CATEGORY_LEEBRARY_SHARED, id: null });
      } else if (
        key === CATEGORY_LEEBRARY_RECENT &&
        category?.key !== CATEGORY_LEEBRARY_RECENT
      ) {
