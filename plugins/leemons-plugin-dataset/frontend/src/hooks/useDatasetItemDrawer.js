...
import {
  getDatasetSchemaFieldLocaleRequest,
...
const ItemDrawer = ({
  ...
  function filterSchemaKeys(schema) {
    ...
    if (schema.selectPlaceholder)
      schemaGoodKeys.selectPlaceholder = schema.selectPlaceholder;
    if (schema.optionLabel) schemaGoodKeys.optionLabel = schema.optionLabel;
    if (schema.yesOptionLabel)
      schemaGoodKeys.yesOptionLabel = schema.yesOptionLabel;
    if (schema.noOptionLabel)
      schemaGoodKeys.noOptionLabel = schema.noOptionLabel;
    if (schema.items?.enumNames)
      schemaGoodKeys.items = { enumNames: schema.items.enumNames };
    ...
    if (schema.frontConfig?.checkboxLabels) {
      schemaGoodKeys.frontConfig = {
        checkboxLabels: schema.frontConfig.checkboxLabels,
      };
    }
  ...
  function prepareSchemaWithAllConfig(data) {
    const schemaWithAllConfig = transformItemToSchemaAndUi(
      data,
      Object.keys(data.locales)[0]
    );
    if (item) {
  ...
  function oneOfCentersHasRole(roleId, schemaWithAllConfig) {
    const selectedCenters = _.filter(
      ...
      ({ id }) =>
        schemaWithAllConfig.schema.frontConfig.centers.indexOf(id) >= 0
    );
    return selectedCenters.some(({ roles }) =>
  ...
  function calculatePermissions(schemaWithAllConfig) {
    const permissionsType = schemaWithAllConfig.schema.frontConfig
      .isAllCenterMode
      ? "profile"
      : "role";
  ...
  async function load() {
    try {
      if (!tLoading) {
        ...
        contextRef.current.drawer.locales = _.map(
          locales,
          ({ name, code }) => ({
            label: name,
            code,
        ...
        _.forEach(contextRef.current.drawer.errorMessages, (value, key) => {
          contextRef.current.drawer.errorMessages[key] = t(
            `errorMessages.${key}`
          );
        });
...
export const useDatasetItemDrawer = () => {
  ...
  return [
    ...
    function drawer(data) {
      return (
        <ItemDrawer onClose={() => setShow(false)} opened={show} {...data} />
      );
    },
  ];
};
