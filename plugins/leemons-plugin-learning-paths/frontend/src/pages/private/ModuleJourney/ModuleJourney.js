...
const ModuleJourney = () => {
  ...
  const activityUrl =
    orderedActivities[0]?.assignable?.roleDetails?.studentDetailUrl
      ?.replace(":id", orderedActivities[0]?.id)
      ?.replace(":user", moduleAssignation?.user);

  const evaluationUrl =
    orderedActivities[0]?.assignable?.roleDetails?.evaluationDetailUrl
      ?.replace(":id", orderedActivities[0]?.id)
      ?.replace(":user", moduleAssignation?.user);
            ...
            <TotalLayoutFooterContainer
              ...
              rightZone={
                <Link to={handleButtonUrl()}>
                  <Button rightIcon={<ChevRightIcon />}>
                    {t("nextActivity")}
                  </Button>
                </Link>
              }
