...
import {
  RESOURCES_DEFAULT_PROPS,
  RESOURCES_PROP_TYPES,
} from "./Resources.constants";
import { resourceStyles } from "./Resources.styles";

async function getResources(ids) {
  const response = await getAssetsByIdsRequest(ids, {
    showPublic: true,
    indexable: 0,
  });
...
const Resources = ({ assignation }) => {
  ...
  const [resources] = useApi(
    getResources,
    assignation?.instance?.assignable?.resources
  );
    ...
    <Box>
      {resources &&
        resources?.map((resource, index, array) => (
          <Box
            onClick={() =>
              window.open(resource.url || resource.cover, "_blank", "noopener")
            }
            key={resource?.id}
            sx={{ cursor: "pointer" }}
          >
