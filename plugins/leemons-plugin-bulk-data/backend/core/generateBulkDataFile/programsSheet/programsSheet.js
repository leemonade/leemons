...
const getSeatsPerCourseString = (program) => {
  ...
  return courses
    .map((course) => `${course.index}:${course.metadata.seats}`)
    .join(", ");
};
  ...
  return sortedCycles
    .map((cycle) => {
      const cycleCourses = cycle.courses
        .map(
          (courseId) => courses.find((course) => course.id === courseId).index
        )
        .join("|");
...
const getSubstagesString = (substages) =>
  substages
    .map((substage) => `${substage.name}|${substage.abbreviation}`)
    .join(", ");
...
const solveReferenceGroupsFields = (program) => {
  ...
  return {
    ...
    groupsPerCourse: program.groups?.length
      ...
      : undefined,
    nameFormat:
      program.groupsMetadata?.customNameFormat ??
...
async function createProgramsSheet({
  workbook,
  centers,
  evaluationSystems,
  ctx,
  ...
  const programsData = await ctx.call(
  ...
  programsData?.forEach((program, i) => {
    ...
    const programObject = {
      ...
      centers: centers.find((center) => center.id === program.centers[0])
        .bulkId,
      evaluationSystem: evaluationSystems.find(
        (system) => system.id === program.evaluationSystem
      ).bulkId,
      ...
      coursesOffset: 0,
      courseCredits: program.credits
        ? getCreditsPerCourseString(program.courses)
        : undefined,
      ...
      cycles: program.cycles?.length
        ? getCyclesString(program.cycles, program.courses)
        : undefined,
      substages: program.substages?.length
        ? getSubstagesString(program.substages)
        : undefined,
      haveKnowledge: booleanToYesNoAnswer(program.hasKnowledgeAreas),
      hasSubjectTypes: booleanToYesNoAnswer(program.hasSubjectTypes),
      hideStudentsToStudents: booleanToYesNoAnswer(
        program.hideStudentsToStudents
      ),
