...
async function createLibraryResourcesSheet({
  ...
  assetDetails.forEach((asset, i) => {
    ...
    const subjectBulkId = subjects.find(
      (s) => s.id === asset.subjects?.[0]?.subject
    )?.bulkId;
    const categoryKey = resourceAssets.find((raw) => raw.id === asset.id)
      .category.key;
    const file = categoryKey === "media-files" ? asset.url : "";
    const canAccess = adminShouldOwnAllAssets
      ? "admin|owner"
      : getCanAccess(asset, users);
