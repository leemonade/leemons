...
function getQuestionsString(testQuestions, questions) {
  const questionsMatching = questions.filter((q) =>
    testQuestions.includes(q.id)
  );
...
const getMetadataConfig = (test) => {
  const { hasInstructions, hasResources } =
    test.providerData.metadata.config ?? {};
  const result = [];
...
async function createTestsSheet({
  ...
  worksheet.columns = [
    ...
    {
      header: "instructionsForTeachers",
      key: "instructionsForTeachers",
      width: 10,
    },
    {
      header: "instructionsForStudents",
      key: "instructionsForStudents",
      width: 10,
    },
  ...
  const notIndexableAssetIds = flatMap(
    testDetails,
    (test) => test.providerData.resources
  );
  ...
  testDetails.forEach((test, index) => {
    ...
    const testObject = {
      ...
      cover: test.cover,
      program:
        programs.find((program) => program.id === test.program)?.bulkId ?? "",
      subjects:
        subjects.find((subject) => subject.id === test.subjects?.[0]?.subject)
          ?.bulkId ?? "",
      type: test.providerData.metadata.type,
      gradable: booleanToYesNoAnswer(test.providerData.gradable),
      useAllQuestions: booleanToYesNoAnswer(
        test.providerData.metadata.filters?.useAllQuestions
      ),
      questions: getQuestionsString(
        test.providerData.metadata.questions,
        questions
      ),
