...
const getAnswersFieldsForMonoResponse = (question, assetDetails) => {
  ...
  answersArray.forEach(
    ({ feedback, text, image, imageDescription, isCorrect, hideOnHelp }, i) => {
      const index = i + 1;

      // Answer string
      let string;
      if (question.hasImageAnswers && image) {
        const imageUrl = assetDetails.find(
        ...
        if (feedback) {
          if (feedback.format === "html") {
            answersFeedback.push(
              `${index}@${turndown.turndown(feedback.text)}`
            );
...
const findMultimediaAsset = (multimediaAssets, question) => {
  if (question.type === "map") {
    return multimediaAssets.find(
      (asset) => asset.id === question.mapProperties.image.id
    ).cover;
  }
  const assetFound = multimediaAssets.find(
    (asset) => asset.id === question.stemResource?.id
  );
...
async function createTestsQuestionsSheet({ workbook, qBanks, ctx }) {
  ...
  worksheet.columns = [
    ...
    {
      header: "answers_feedback_image",
      key: "answers_feedback_image",
      width: 20,
    },
  ...
  qBanks.forEach(({ providerData, bulkId: qBankBulkId }) => {
    ...
    questions?.forEach((question) => {
      questionCounter++;
      const stemResource = findMultimediaAsset(
        allMultimediaAssetDetails,
        question
      );
      ...
      const commonData = {
        ...
        qbank: qBankBulkId,
        type: question.type,
        category: qBankCategories.find((c) => c.id === question.category)
          ?.value,
