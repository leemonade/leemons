...
const getCreator = (asset, users) =>
  users.find((u) => u.id === asset.fromUser)?.bulkId;
...
function processLibraryTag({
  ...
  const regex =
    /<library ([^>]*?)id="([^"]+?@\d+\.\d+\.\d+)"([^>]*?)>.*?<\/library>/g;
  return htmlContent.replace(
    regex,
    (match, preIdAttributes, id, postIdAttributes) => {
      const asset = embededAssets.find(
        (embededAsset) => embededAsset.id === id
      );
...
async function createContentCreatorSheet({
  ...
  const embededAssetIds = getEmbededAssetsFromHTML(
    assignableDetails.map((item) => item.content)
  );
  ...
  return documentDetails.map((document, i) => {
    const assignable = assignableDetails.find(
      (item) => item.id === document.providerData.id
    );
