...
const getCalendarReferenceString = ({
  ...
  const calendarIds = Array.isArray(_calendarIds)
    ? _calendarIds
    : [_calendarIds];
  ...
  return calendarIds
    .map((calendarId) => {
      ...
      if (calendar.isClass) {
        ...
        const classIndex = sortedClasses
          .map((cls) => cls.id)
          .indexOf(classIdFromKey);
      ...
      if (calendar.isUserCalendar) {
        ...
        const user = users.find((u) =>
          u.userAgents.map((ua) => ua.id).includes(userAgent)
        );
        return user.bulkId;
      }
      return "";
...
const getSubtaskString = (subtasks) =>
  (subtasks || []).map((item) => item?.title).join("|");

const procesTaskData = ({
  event,
  userCalendars,
  subjects,
  users,
...
const getCreator = (event, users) => {
  ...
  const creator = users.find((user) =>
    user.userAgents.map((ua) => ua.id).includes(owners[0])
  );
  return creator?.bulkId;
};

async function createCalendarSheet({
  workbook,
  subjects,
  users,
  noUsers,
