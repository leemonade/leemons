...
async function createRegionalCalendarsSheet({ workbook, centers, ctx }) {
  ...
  centers.forEach((center) => {
    regionalCalendarPromises.push(
      ctx
        .call("academic-calendar.regionalConfig.listRest", {
          center: center.id,
        })
        .then((result) => result.regionalConfigs)
    );
  });
  const regionalCalendars = flatten(
    await Promise.all(regionalCalendarPromises)
  );
  ...
  return mapWithBulkIds.map((rg) => {
    const finalObject = {
      ...
      regionalEventsRel: mapWithBulkIds.find(
        (_rg) => _rg.id === rg.regionalEventsRel
      )?.bulkId,
    };
