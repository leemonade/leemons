    ...
    (
      ...
      if (question.properties?.explanationInResponses) {
        ...
        answersFeedback.push(
          turndown.turndown(question.properties.explanation ?? "")
        );
      }
    }
  );
...
const findQuestionImage = (imageAssets, question) => {
  if (question.type === "map") {
    return imageAssets.find(
      (asset) => asset.id === question.properties.image.id
    ).cover;
  }
  return imageAssets.find((asset) => asset.id === question.questionImage?.id)
    ?.cover;
};
