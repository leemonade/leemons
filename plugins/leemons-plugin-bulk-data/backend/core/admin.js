...
async function initAdmin({ file, ctx }) {
  ...
  try {
    ...
    Object.keys(appearanceSettings).forEach((key) => {
      ...
      promises.push(
        ctx.call("users.platform.setAppearanceMainColor", {
          value: settings.mainColor,
        })
      );
      promises.push(
        ctx.call("users.platform.setAppearanceMenuMainColor", {
          value: settings.menuMainColor,
        })
      );
      promises.push(
        ctx.call("users.platform.setAppearanceMenuDrawerColor", {
          value: settings.menuDrawerColor,
        })
      );

      if (settings.logoUrl)
        promises.push(
          ctx.call("users.platform.setLandscapeLogo", {
            value: settings.logoUrl,
          })
        );
      if (settings.squareLogoUrl)
        promises.push(
          ctx.call("users.platform.setSquareLogo", {
            value: settings.squareLogoUrl,
          })
        );
      if (settings.emailLogoUrl)
        promises.push(
          ctx.call("users.platform.setEmailLogo", {
            value: settings.emailLogoUrl,
          })
        );
      if (settings.emailWidthLogo)
        promises.push(
          ctx.call("users.platform.setEmailWidthLogo", {
            value: settings.emailWidthLogo,
          })
        );
    });
