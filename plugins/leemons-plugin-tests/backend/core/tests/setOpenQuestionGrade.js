const {
  calculateUserAgentInstanceNote,
} = require("./calculateUserAgentInstanceNote");
...
async function setOpenQuestionGrade({ data, ctx }) {
  ...
  const instanceClasses = await ctx.tx.call(
  ...
  const { note, questions: gradedQuestions } =
    await calculateUserAgentInstanceNote({
      instanceId,
      userAgent: studentUserAgentId,
      ctx,
    });
  ...
  if (teacherFeedback) {
    updatedProperties = {
      ...(currentResponse.properties
        ? JSON.parse(currentResponse.properties)
        : {}),
      teacherFeedback,
    };
  }
  ...
  await ctx.tx.call("assignables.assignations.updateAssignation", {
    assignation: {
      ...
      metadata: {
        ...
        score: {
          count: Object.values(gradedQuestions).filter(
            ({ status }) => status === "ok"
          ).length,
