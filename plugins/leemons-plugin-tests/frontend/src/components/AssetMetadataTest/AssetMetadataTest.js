...
const AssetMetadataTest = ({ metadata, canEdit }) => {
  ...
  const getQuestionBankData = () =>
    getQuestionBankRequest(metadata?.providerData?.metadata?.questionBank).then(
      (res) => setData(res.questionBank)
    );
  useEffect(() => {
  ...
  const { classes } = AssetMetadataTestStyles(
    {},
    { name: "AssetMetadataTest" }
  );
  ...
  const getFieldsToRender = (nameTest, questionsTest, categoriesTest) => {
    ...
    const cuestionsNumber = Array.isArray(questionsTest)
