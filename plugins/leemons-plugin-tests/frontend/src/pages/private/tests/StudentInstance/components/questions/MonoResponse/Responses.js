...
export default function Responses(props) {
  ...
  const components = question.choices.map((choice, index) => {
    ...
    if (index === currentResponseIndex) {
      classContainer = cx(
        classContainer,
        styles.questionResponseImageContainerSelected
      );
    }
    ...
    if (!question.hasImageAnswers) {
      ...
      classDisableIcon = cx(
        classDisableIcon,
        styles.disableResponseIconWithOutImage
      );
    }
    ...
    if (clued) {
      classContainer = cx(
        classContainer,
        styles.questionResponseImageContainerClued
      );
    }
    ...
    if (store.viewMode) {
      classContainer = cx(
        classContainer,
        styles.questionResponseImageContainerViewMode
      );

      if (question.hasAnswerFeedback) {
        const text = choice.feedback?.text
          ? htmlToText(choice.feedback.text)
          : null;
      ...
      if (choice.isCorrect) {
        ...
        classContainer = cx(
          classContainer,
          styles.questionResponseImageContainerDone
        );
        ...
        classContainer = cx(
          classContainer,
          styles.questionResponseImageContainerWrong
        );
        classExplanation = cx(classExplanation, styles.textExplanationWhite);
      } else {
        classContainer = cx(
          classContainer,
          styles.questionResponseRemovePadding
        );
        classExplanation = cx(
          classExplanation,
          ...
          <Box>
            <Stack fullWidth alignItems="center">
              {!store.viewMode ? (
                <Radio
                  checked={
                    ...
                    (store.viewMode && choice.isCorrect)
                  }
                  noRootPadding
                />
              ) : null}
              ...
              {!store.viewMode ? (
                <Box
                  sx={(theme) => ({
                    marginLeft: store.viewMode ? 0 : theme.spacing[4],
                  })}
