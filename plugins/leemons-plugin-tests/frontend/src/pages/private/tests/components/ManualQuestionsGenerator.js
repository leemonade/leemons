...
const ManualQuestionsGenerator = ({
  ...
  useEffect(() => {
    ...
    if (formValues?.config?.manualQuestions?.length > 0) {
      setManualQuestions(
        assignmentMode
          ? assignmentQuestions.filter((q) =>
              formValues.config.manualQuestions.includes(q.id)
            )
          : questionBank?.questions?.filter((q) =>
              formValues.config.manualQuestions.includes(q.id)
            )
      );
  ...
  return (
    <Box>
      <Title
        order={4}
        style={{ visibility: questions?.length > 0 ? "visible" : "hidden" }}
      >
