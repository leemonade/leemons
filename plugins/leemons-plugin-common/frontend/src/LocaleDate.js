...
export function getLocaleDuration(
  { seconds: secondsProp, short = false, options = {} },
  session
) {
  ...
  if (!rFormatters[key]) {
    rFormatters[key] = new Intl.RelativeTimeFormat(
      [locale, "default"],
      options
    );
  }
...
export function LocaleRelativeTime({
  ...
  const result = formatter.format(
    Math.floor(seconds / unit.seconds),
    unit.unit
  );
...
export function LocaleWeekDay({ day }) {
  ...
  if (!formatters[key]) {
    formatters[key] = new Intl.DateTimeFormat([locale, "default"], {
      weekday: "long",
    });
  }
...
export function getLocaleFormat(locale) {
  ...
  const formattedDate = new Intl.DateTimeFormat([locale, "default"]).format(
    testDate
  );
