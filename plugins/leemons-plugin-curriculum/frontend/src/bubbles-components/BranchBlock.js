...
function TypeOfRelation({ hasParent, messages, selectData, ...props }) {
  ...
  const relatedToValue = props.form.getValues(
    props.name.replace("typeOfRelation", "relatedTo")
  );
  if (relatedToValue) {
...
function ShowNumeration({ selectData, ...props }) {
  ...
  // eslint-disable-next-line react/prop-types
  const relatedToValue = props.form.getValues(
    props.name.replace("showNumeration", "relatedTo")
  );
  if (relatedToValue) {
...
function BranchBlock({
  ...
  useEffect(() => {
    const subscription = watch(
      ({ curricularContent, name, type }, { name: n }) => {
        if (n === "type") {
      ...
      render={({ field }) => (
        <Select
          ...
          data={[
            ...
            {
              label: messages.curricularQualifyingCriteria,
              value: "qualifying-criteria",
            },
            {
              label: messages.curricularNonQualifyingCriteria,
              value: "non-qualifying-criteria",
            },
          ]}
          {...field}
        />
  ...
  const listOrdered = (
    <>
      <Controller
        ...
        render={({ field }) => (
          <Switch
            label={messages.useNumerationLabel}
            checked={field.value}
            {...field}
          />
        )}
    ...
    function getNumerationValues(blockData) {
      ...
      if (
        ...
        (blockData.type === "group" &&
          types.includes(blockData.groupListOrdered))
      ) {
        result.push({
      ...
      forEach(props, (prop, key) => {
        data.push({
          ...
          isParent:
            ...
            formData.curricularContent ===
              prop.frontConfig.blockData.curricularContent,
        ...
        },
        {
          ...
          input: {
            node: (
              <TypeOfRelation
                hasParent={
                  !!find(formData.contentRelations, {
      ...
      <Box className={classes.container}>
        ...
        <ContextContainer>
          ...
          <Controller
            ...
            render={({ field }) => (
              <Switch
                label={messages.useContentRelations}
                checked={field.value}
                {...field}
              />
            )}
