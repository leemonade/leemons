    ...
    return (
      <Box key={i} /* sx={(theme) => ({ marginTop: theme.spacing[4] })} */>
        ...
        {groupByProperty.map((propertyValues, x) => {
          const propertyLabel =
            store.data.nodeLevels[propertyValues[0].nodeLevel].compileJsonSchema
              .properties[propertyValues[0].property].title;
          return (
            <Box
              key={x}
              sx={(theme) => ({
            ...
            >
              ...
              <InputWrapper label={propertyLabel}>
                {propertyValues.map((propertyValue, j) => {
                  const formValue =
                    store.data.nodes[nodeValues[0].node].formValues[
                      propertyValue.property
                    ];
                  const formValues = isArray(formValue)
                    ? formValue
                    : [formValue];
