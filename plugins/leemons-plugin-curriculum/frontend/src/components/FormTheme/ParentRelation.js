...
const ParentRelation = ({
  ...
  const parentNodes = React.useMemo(
    () => getParentNodes(curriculum.nodes, id),
    [curriculum]
  );
  ...
  const parentRelatedValueText = React.useMemo(() => {
    if (store.parentNodeValue) {
      if (store.selectData) {
        const found = _.find(store.selectData, {
      ...
      if (_.isArray(store.parentNodeValue)) {
        return _.find(store.parentNodeValue, {
          id: props.value?.metadata?.parentRelated,
        })?.value;
      }
  ...
  function print() {
    if (isEditMode) {
        ...
        return (
          <TextInput
            ...
            label={
              hideLabel
                ? null
                : t("parentBlock", { name: store.selectParentName })
            }
          />
        );
      }
      ...
      if (store.type === "select" && props.value?.metadata?.parentRelated) {
        const item = _.find(store.selectData, {
          value: props.value?.metadata?.parentRelated,
        });
        if (item) {
          return (
