...
export function getItemTitleNumbered(
  blockData,
  values,
  index,
  onlyNumber,
  ...
  if (values?.metadata?.initNumber) {
    if (
      blockData.listOrdered === "style-1" ||
      blockData.groupListOrdered === "style-1"
    ) {
      return values.metadata.initNumber + index;
    }
    if (
      blockData.listOrdered === "style-2" ||
      blockData.groupListOrdered === "style-2"
    ) {
      return numberToEncodedLetter(values.metadata.initNumber + index);
    }
    if (
      blockData.listOrdered === "custom" ||
      blockData.groupListOrdered === "custom"
    ) {
