...
async function getDataForKeys({ keys, ctx }) {
  ...
  const values = _.map(keys, (key) =>
    getCurriculumSelectedContentValueByKey(key)
  );
  ...
  const [_nodes, schemas, nodeValues] = await Promise.all([
    ...
    Promise.all(
      _.map(nodeIds, (nodeId) =>
        getNodeValues({
          node: { id: nodeId, nodeLevel: valuesByNode[nodeId].nodeLevel },
          ctx,
        })
      )
    ),
  ...
  _.forEach(nodeLevelIds, (nodeLevelId) => {
    nodeLevels[nodeLevelId] =
      schemasByLocationName[`node-level-${nodeLevelId}`] || null;
  });
  _.forEach(_nodes, (node) => {
    nodes[node.id] = node;
  });
