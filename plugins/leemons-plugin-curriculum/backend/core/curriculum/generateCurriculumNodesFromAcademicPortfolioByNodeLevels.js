...
async function generateCurriculumNodesFromAcademicPortfolioByNodeLevels({
  ...
  const [tree, currentNodes] = await Promise.all([
    ...
    ctx.tx.db.Nodes.find({ curriculum: curriculum.id })
      .select(["id", "treeId"])
      .lean(),
  ]);
      ...
      if (child.nodeType !== "class") {
        ...
        if (currentTreeIds.includes(child.treeId)) {
          ...
          node = await ctx.tx.db.Nodes.create({
            name:
              child.value && child.value.name ? child.value.name : "undefined",
            nodeOrder: i,
  ...
  if (tree.length) {
    ...
    const treeIdsToRemove = _.filter(
      currentTreeIds,
      (id) => !treeIds.includes(id)
    );
