...
const FeedbackList = loadable(() =>
  pMinDelay(import("./src/pages/private/feedback/List"), 500)
);
...
const Result = loadable(() =>
  pMinDelay(import("./src/pages/private/feedback/Result"), 500)
);

const Preview = loadable(() =>
  pMinDelay(import("./src/pages/private/feedback/Preview"), 500)
);
  ...
  return (
    <Switch>
      ...
      <Route path={`${path}/draft`}>
        <Redirect
          to={"/private/leebrary/assignables.feedback/list?activeTab=draft"}
        />
      </Route>
      <Route path={`${path}/:id`}>
        <FeedbackDetail
