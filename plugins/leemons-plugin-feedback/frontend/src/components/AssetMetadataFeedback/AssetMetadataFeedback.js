...
const AssetMetadataFeedback = ({ metadata }) => {
  ...
  const getQuestionSurveyData = () =>
    getFeedbackRequest(metadata?.providerData?.id).then((res) =>
  ...
  const { classes } = AssetMetadataFeedbackStyles(
    {},
    { name: "AssetMetadataFeedback" }
  );
  const getFieldsToRender = (dataSurvey) => {
    const cuestionsNumber = Array.isArray(dataSurvey.questions)
    ...
    if (
      Array.isArray(dataSurvey.questions) &&
      dataSurvey.questions.length >= 1
    ) {
