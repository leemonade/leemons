...
module.exports = {
  ...
  events: {
    "users.init-profiles": async (ctx) => {
      const profiles = await ctx.call(
        "academic-portfolio.settings.getProfiles"
      );
      ...
      await ctx.call("widgets.widgets.updateOrderItemsInZone", {
        items: itemsToUpdate,
      });
      await ctx.call("widgets.widgets.updateProfileItemsInZone", {
        items: itemProfiles,
      });
