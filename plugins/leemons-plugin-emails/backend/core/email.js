...
class Email {
  ...
  static async getProvider({ pluginKeyValue, ctx }) {
    const providers = await ctx.tx.call(
      `${pluginKeyValue.pluginName}.email.getProviders`
    );
    return {
  ...
  static async providers({ ctx }) {
    const providers = [];
    const plugins = await getPluginProviders({
  ...
  static async saveProvider({ ctx, providerName, config }) {
    const provider = await getPluginProvider({
      keyValueModel: ctx.tx.db.KeyValue,
      providerName,
    });
    if (!provider)
      throw new LeemonsError(ctx, {
        message: `No provider with the name ${providerName}`,
      });
  ...
  static async add({ templateName, language, subject, html, type, ctx }) {
    ...
    let template = await db.EmailTemplate.findOne({ templateName })
      .select(["id"])
      .lean();
  ...
  static async delete({ templateName, language, type, ctx }) {
    const template = await ctx.tx.db.EmailTemplate.findOne({ templateName }, [
      "id",
    ]).lean();
    if (!template)
      throw new LeemonsError(ctx, {
  ...
  static async send({
    ...
    if (!providers.length) {
      throw new LeemonsError(ctx, {
        message: "No email providers configured yet",
      });
    }
  ...
  static async sendAsPlatform({ to, templateName, language, context, ctx }) {
    return Email.sendAsEducationalCenter({
      to,
      templateName,
      language,
      context,
    ...
    try {
      ...
      if (!provider) {
        if (errIfNotProvider) {
          return {
            error: true,
            message: "Could not send email with any provider",
          };
        }
      ...
      const info = await ctx.tx.call(
        `${provider.providerName}.email.sendMail`,
        {
          ...receipts,
          subject: email.subject,
          html: email.html,
