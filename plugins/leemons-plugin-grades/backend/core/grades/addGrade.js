...
async function addGrade({ data, fromFrontend, ctx }) {
  ...
  if (!fromFrontend) {
    const newScales = await Promise.all(
      _.map(scales, (scale) =>
        addGradeScale({ data: { ...scale, grade: grade.id }, ctx })
      )
    );
    ...
    if (!minScale)
      throw new LeemonsError(ctx, {
        message: "minScaleToPromote not found inside scales",
      });

    await ctx.tx.db.Grades.updateOne(
      { id: grade.id },
