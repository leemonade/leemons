...
async function update({
  ...
  if (center) {
    const rolesInCenter = await ctx.tx.db.RoleCenter.find({ center })
      .select(["role"])
      .lean();
  ...
  const [role, roleCenter] = await Promise.all([
    ctx.tx.db.Roles.findByIdAndUpdate(
      id,
  ...
  if (_.isNull(center) || center)
    await ctx.tx.db.RoleCenter.deleteOne({ id: roleCenter.id });
