...
function Kanban({ session }) {
  ...
  async function getKanbanColumnsEventsOrder() {
    const { orders } = await listKanbanEventOrdersRequest(
  ...
  function getKanbanBoard() {
    ...
    _.forEach(ref.current.columns, (column) => {
      if (
        !column.isArchived ||
        (column.isArchived && ref.current.filters.showArchived)
      ) {
        let cards = [];
        if (
          ...
          _.forEach(eventsByColumn[column.id], (event) => {
            const index = ref.current.columnsEventsOrders[column.id].indexOf(
              event.id
            );
            if (index >= 0) {
              cards[index] = event;
        ...
        if (ref.current.filters.onlyByMy) {
          const { userAgentId } = _.find(getCentersWithToken(true), {
            id: ref.current.center.id,
          });
        ...
        cols.push({
          ...
          cards: _.map(cards, (card) =>
            transformEv(card, ref.current.data.calendars)
          ),
        });
      }
    });
  ...
  if (ref.current.board) {
    const calendarIds = uniq(
      flatten(
        map(ref.current.board.columns, (column) =>
          map(column.cards, "calendar")
        )
      )
    );
