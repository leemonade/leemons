...
describe("Upload S3 Multipart Chunk", () => {
  ...
  it("Should correctly upload multipart chunk and return true", async () => {
    ...
    // Act
    const response = await uploadMultipartChunk({
      file,
      partNumber,
      buffer,
      path,
  ...
  it("Should throw an error if no started multipart upload for this file", async () => {
    ...
    // Act and Assert
    await expect(
      uploadMultipartChunk({ file, partNumber, buffer, path, ctx })
    ).rejects.toThrow("No started multipart upload for this file");
  });
