...
export default function MainMenu({ subNavWidth, ...props }) {
  ...
  async function load() {
    ...
    const [{ centers }, { profiles }, { name }, { locales }] =
      await Promise.all([
    ...
    const filteredCenters = centers.map((center) => ({
      ...center,
      profiles: center.profiles.filter(
  ...
  const sessionMenuData = React.useMemo(() => {
    ...
    t,
    store.hasManyLocales,
    deploymentConfig?.helpdeskUrl,
