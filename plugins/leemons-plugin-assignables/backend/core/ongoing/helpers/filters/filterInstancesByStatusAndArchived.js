...
function filterInstancesByStatusAndArchived({
  instances,
  filters,
  dates,
  hideNonVisible,
  ...
  const filterByStatus = ["open", "closed", "scheduled"].includes(
    desiredStatus
  );
  ...
  if (filterByStatus) {
    ...
    filteredInstances = filteredInstances.filter(
      (instance) => !!instancesIdsToReturn[instance.id]
    );
  }
  ...
  if (filterByArchived) {
    ...
    filteredInstances = filteredInstances.filter((instance) => {
      const now = dayjs();
      const archivedDate = dayjs(
        datesPerInstance[instance.id]?.archived || null
      );

      const instanceIsArchived =
        archivedDate.isValid() && !now.isBefore(archivedDate);
