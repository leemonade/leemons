...
jest.mock('../getAssignations', () => ({
  getAssignations: () => Promise.resolve([
    {
      instance: 'instance1',
...
it('Should return error when all not edit', async () => {
  ...
  // Act and Assert
  await expect(getAssignationsOfInstance({ instances, details: true, ctx })).rejects.toThrow();
});
...
it('Should return all assignations for given instances with details true', async () => {
  ...
  // Act
  const assignations = await getAssignationsOfInstance({ instances, details: true, ctx });
