...
it('Should get assignations', async () => {
  ...
  // Act
  const result = await getAssignations({ assignationsIds, throwOnMissing: false, fetchInstance: true, ctx });
...
it('Should error when not permissions', async () => {
    ...
    // Act
    // Assert
    await expect(getAssignations({ assignationsIds, throwOnMissing: true, ctx })).rejects.toThrowError();
  });