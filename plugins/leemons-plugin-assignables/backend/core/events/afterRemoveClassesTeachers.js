...
async function afterRemoveClassesTeachers({ classTeachers, ctx }) {
  if (classTeachers?.length) {
    ...
    _.forEach(assignations, (assignation) => {
      const assignationClasses = _.map(
        assignation.classes,
        (classId) => classesById[classId]
      );
      const SUBJECT_ID = "subject.id";
      const classesBySubject = _.groupBy(assignationClasses, SUBJECT_ID);
      const subjectIds = _.map(
        _.uniqBy(assignationClasses, SUBJECT_ID),
        SUBJECT_ID
      );
      ...
      _.forEach(subjectIds, (subjectId) => {
        ...
        const teachersRemoved = _.map(
          _.filter(classTeachers, (classTeacher) =>
            _.includes(_classIds, classTeacher.class)
          ),
          "teacher"
        );
        ...
        _.forEach(_classes, (data) => {
          _.forEach(data.teachers, (teacher) => {
            if (teacherTypes.includes(teacher.type))
        ...
        _.forEach(teachersRemoved, (teacherId) => {
          if (!_.includes(currentTeachers, teacherId))
            teachersToRemove.push(teacherId);
        });
