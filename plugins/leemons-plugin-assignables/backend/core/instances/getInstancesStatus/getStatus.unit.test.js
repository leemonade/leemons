...
it("Should return the correct status based on student and instance data", () => {
  ...
  const responseLate = getStatus(
    {
      ...studentData,
      timestamps: { ...studentData.timestamps, end: "2022-02-01" },
      grades: [],
    },
    instanceData
  );
  const responseSubmitted = getStatus(
    { ...studentData, grades: [] },
    instanceData
  );
  const responseClosed = getStatus(
    {
      ...studentData,
      timestamps: { ...studentData.timestamps, end: undefined },
      grades: [],
    },
    instanceData
  );
  ...
  const responseAssigned = getStatus(
    ...
    {
      ...instanceData,
      dates: { ...instanceData.dates, deadline: undefined, start: undefined },
    }
  );
