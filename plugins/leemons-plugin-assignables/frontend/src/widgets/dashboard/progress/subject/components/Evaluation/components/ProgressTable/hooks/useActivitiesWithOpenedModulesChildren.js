...
function parseChildAssignation({ activity, childrenAssignationsByModuleId }) {
  ...
  const childrenAssignations =
    childrenAssignationsByModuleId[activity.instance.id] ?? [];

  const calificableChildren = filter(childrenAssignations, "instance.gradable");
  const weightPerCalificable = calificableChildren.length
    ? weight / calificableChildren.length
    : 0;

  const childrenWithWeightsAndScore = childrenAssignations.map(
    (assignation) => {
      const mainGrade = assignation.grades?.find(
        (grade) => grade.type === "main"
      );
      const activityWeight = assignation.instance.gradable
        ? weightPerCalificable
        : 0;
      return {
        ...assignation,
