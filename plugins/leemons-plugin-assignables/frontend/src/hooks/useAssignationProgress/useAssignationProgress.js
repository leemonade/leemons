 ...
 */
export default function useAssignationProgress({
  assignation = {},
  isBlocked,
}) {
  ...
  const hasAllGrades =
    mainGrades?.length === subjectsCount && subjectsCount > 0;
  const hasBeenEvaluated = isEvaluable && hasAllGrades;

  const averageGrade =
    hasAllGrades && mainGrades?.length
      ? mainGrades?.reduce((acc, { grade }) => acc + grade, 0) /
        mainGrades?.length
      : null;
  ...
  const nearestScale = evaluationSystem
    ...
    : null;
  const nearestScaleDisplay =
    nearestScale?.letter ?? nearestScale?.number ?? null;
  ...
  return {
    statusName,
    label: isEvaluated
      ? `${t(status.labelKey)} (${nearestScaleDisplay})`
      : t(status.labelKey),
    color,
