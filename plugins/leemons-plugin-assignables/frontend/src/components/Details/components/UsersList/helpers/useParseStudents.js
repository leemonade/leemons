...
function useStudentData(students) {
  ...
  return useMemo(() => {
    ...
    return students.map((student) => {
      const userAgent = userAgentsInfoMulti.data.find(
        (d) => d.id === student.user
      );
      return {
        ...student,
...
function getStudentAverageScore(studentData) {
  ...
  const mainScores = scores.filter(
    (grade) => grade.type === "main" && !isNil(grade.grade)
  );
    ...
    return students?.map((student) => {
      ...
      return {
        ...
        unreadMessages: (
          <UnreadMessages
            rooms={
              instance?.metadata?.createComunicaRooms ? student.chatKeys : []
            }
          />
        ),
        ...
        avgTime:
          student?.timestamps?.start && student?.timestamps?.end ? (
            <LocaleDuration
              seconds={dayjs(student.timestamps.end).diff(
                student.timestamps.start,
                "seconds"
              )}
            />
          ) : (
