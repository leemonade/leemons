...
export default function useDatesPickerOptions() {
  ...
  const {data: localizations, isLoading: isLoadingLocalizations} = useWidgetLocalizations({widgets});


  const options = useMemo(() => widgets.map((widget) => ({
      value: widget.id,
  ...
  const components = useMemo(() => widgets.reduce((acc, widget) => {
