...
export function useOngoingData({ query, page, size, subjectFullLength }) {
  ...
  const { data: paginatedInstances, isLoading: instancesLoading } =
  ...
  const { isLoading: instancesDataLoading, data: instancesData } =
    useAssignationsByProfile(instances || [], {
      placeholderData: [],
    });
  ...
  const orderedInstancesData = useMemo(() => {
    ...
    instancesData?.forEach((instance) => {
      sortedData[originalOrder[instance.instance?.id || instance.id]] =
        instance;
    });
  ...
  const { data: parsedInstances, isLoading: parsedInstancesLoading } =
    useParseAssignations(orderedInstancesData, {
      subjectFullLength,
      modulesOpened,
    ...
    () =>
      parsedInstances
        ?.filter(
          (instance) =>
            !!instance &&
            (!instance.parentModule ||
              modulesOpened.includes(instance.parentModule))
        )
        .map((instance) => ({
          ...instance,
  ...
  const isLoading =
    instancesLoading || instancesDataLoading || parsedInstancesLoading;
