...
const useStatus = ({ instance }) => {
  ...
  return useMemo(() => {
    ...
    const isClosed =
      (closed && !now.isBefore(dayjs(closed ?? null))) || deadlinePassed;
    const isArchived = archived && !now.isBefore(dayjs(archived ?? null));
    const isOpen =
      alwaysAvailable || (start && !dayjs(start ?? null).isAfter(now));
...
const useBadgeStyle = ({ status }) =>
  useMemo(() => {
    ...
    return {
      badge: "openActivityBadge",
      text: "openActivityBadgeText",
      label: "opened",
    };
  }, [status]);
...
const StatusBadge = ({ instance }) => {
  ...
  return (
    <Badge className={classes[style.badge]} closable={false} size="xs">
      <Text className={classes[style.text]}>
        {t(style.label).toUpperCase()}
      </Text>
    </Badge>
  );
};
