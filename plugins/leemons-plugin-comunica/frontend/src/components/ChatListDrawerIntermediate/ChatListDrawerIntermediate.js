...
function ChatListDrawerIntermediate({
  ...
  const { classes } = ChatListDrawerIntermediateStyles(
    {},
    { name: "ChatListDrawerIntermediate" }
  );
  ...
  const rooms = React.useMemo(() => {
    ...
    if (room.type === "assignables.assignation") {
      const subjectRooms = _.filter(results, {
        type: "assignables.assignation.subject",
      });
      // Si solo tenemos una sala de tipo asignatura pintamos directamente sus salas hijas
      if (subjectRooms.length === 1) {
        results = _.filter(
          results,
          ({ type }) => type !== "assignables.assignation.subject"
        );
        const subjectChildrens = getRoomChildrens(
          room.childrens,
          subjectRooms[0]
        );
