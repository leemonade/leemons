...
async function toggleDisableRoom({ key, userAgent, ctx }) {
  ...
  if (!userAgentRoom.isAdmin)
    throw new LeemonsError(ctx, {
      message: "You don`t have permissions for disable this room",
    });

  const room = await ctx.tx.db.Room.findOne({ key })
    .select(["adminDisableMessages"])
    .lean();

  await ctx.tx.db.Room.updateOne(
    { key },
