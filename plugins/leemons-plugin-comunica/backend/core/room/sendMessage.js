...
async function sendMessage({
  ...
  if (message.type === "text") {
    const center = await ctx.tx.call("users.users.getUserAgentCenter", {
      userAgent: _userAgent,
    });
    const centerConfig = await getCenter({ center: center.id, ctx });
    if (centerConfig.enableSecureWords) {
      const words = _.map(centerConfig.secureWords.split(","), (word) =>
        word.trim()
      );
  ...
  let response = await ctx.tx.db.Message.create({
    ...
    message: JSON.stringify(
      room.useEncrypt ? encrypt(message, userAgent.encryptKey) : message
    ),
  ...
  _.forEach(userAgentIds, (userAgentId) => {
    promises.push(
      setMessageUnRead({ key, userAgentId, message: response.id, ctx })
    );
  });
