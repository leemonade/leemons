...
function Setup({ session }) {
  ...
  React.useEffect(() => {
    if (
      ...
      navigate(deploymentConfig?.superRedirectUrl, { replace: true });
    }
  }, [JSON.stringify(deploymentConfig), navigate]);
  ...
  const stepperProps = React.useMemo(() => {
    ...
    return {
      completedSteps: [...new Array(store.steps + 1).keys()],
      visitedSteps: [...new Array(store.steps + 1).keys()],
    };
  }, [store.configured]);
