...
export function getScormDuration({ state, assignation }) {
  ...
  if (cmi2004TotalTime) {
    hours =
      new RegExp(/(?<hours>\d*)H/).exec(cmi2004TotalTime)?.groups?.hours ?? "0";
    minutes =
      new RegExp(/(?<minutes>\d*)M/).exec(cmi2004TotalTime)?.groups?.minutes ??
      "0";
    seconds =
      new RegExp(/(?<seconds>\d*)S/).exec(cmi2004TotalTime)?.groups?.seconds ??
      "0";
