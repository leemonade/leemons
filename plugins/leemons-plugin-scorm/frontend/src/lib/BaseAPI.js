...
export default class BaseAPI {
  ...
  #settings = {
    ...
    onLogMessage(messageLevel, logMessage) {
      switch (messageLevel) {
  ...
  setValue(callbackName, commitCallback, checkTerminated, CMIElement, value) {
    ...
    if (
      this.checkState(
        checkTerminated,
        this.#error_codes.STORE_BEFORE_INIT,
  ...
  commit(callbackName, checkTerminated) {
    ...
    if (
      this.checkState(
        checkTerminated,
        this.#error_codes.COMMIT_BEFORE_INIT,
  ...
  checkState(checkTerminated, beforeInitError, afterTermError) {
    ...
    if (checkTerminated && this.isTerminated()) {
        ...
        if (!isNaN(index)) {
          ...
          if (item) {
            ...
            this.throwSCORMError(
              this.#error_codes.VALUE_NOT_INITIALIZED,
  ...
  loadFromFlattenedJSON(json, CMIElement) {
    ...
    const result = Object.keys(json).map((key) => [String(key), json[key]]);
