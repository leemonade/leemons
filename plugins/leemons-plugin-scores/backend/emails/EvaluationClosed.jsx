...
function EvaluationClosedEmail({
  ...
  return (
    <EmailLayout previewText={previewText} title={t.title} locale={locale}>
      ...
      <Container className="text-center">
        ...
        <Container className="bg-[#F2F4F8] rounded-sm mb-[10px] mt-[10px] w-[160px] mx-auto border-solid border-[#DDE1E6] border">
          ...
          <Text className="mb-0 font-bold">
            <span style={{ fontSize: "48px", lineHeight: "48px" }}>
              {gradeInt}
            </span>
            {ifGradeDecimals}
          ...
          {endIf}
          {ifGradeLetter}
          <Text className="mb-0 font-bold text-[48px] leading-[48px]">
            {gradeLetter}
          </Text>
          {endIf}
      ...
      <Text className="text-[12px] text-gray-500 mt-6 mb-0 text-center">
        {t.reminderText}
      </Text>
