...
export default function Footer({ isCustom, disableReports }) {
  ...
  const setIsPeriodPublished = useEvaluationNotebookStore(
    (state) => state.setIsPeriodPublished
  );
  ...
  useEffect(() => {
    ...
    tableData?.activitiesData?.value,
    isPeriodPublished,
    setIsPeriodPublished,
  ]);

  if (
    !tableData?.activitiesData?.activities?.length ||
    !tableData?.activitiesData?.value?.length
  ) {
    return null;
  }
