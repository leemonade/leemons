...
export default function useActivities({
  ...
  const query = useMemo(
    () => ({
      ...
      finished: true,
      finished_$gt: getPreviousDayLastMillisecond(
        period?.startDate
      ).toISOString(),
  ...
  const { data: instancesIds, isLoading: instancesIdsLoading } =
  ...
  return {
    activities,
    isLoading:
      instancesIdsLoading || (instancesIds?.length && instancesLoading),
  };
}
