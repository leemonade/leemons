...
export default function Weighting({ class: klass }) {
  ...
  const { data: typesData, isLoading: typesLoading } = useRolesData({
    class: classId,
  });
  const { data: modulesData, isLoading: modulesLoading } = useModulesData({
    class: classId,
  });
  ...
  const data = useMemo(() => {
    ...
    return {
      weights: weightsData,
      applySameValue:
        weights?.type === type ? weights?.applySameValue : undefined,
    };
