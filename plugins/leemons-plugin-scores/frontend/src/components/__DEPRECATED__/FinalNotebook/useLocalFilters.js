...
function filterClassesByLocalFilters({ classes, filters }) {
  ...
  return classes?.filter((klass) =>
    stringMatch(
      klass.groups.name || klass.groups.abbreviation,
      filters.search,
      { partial: true }
    )
  );
...
function filterPeriodsByLocalFilters({ filters, periods }) {
  ...
  return periods?.filter((period) => {
    if (
      !filters?.futureEvaluations &&
      dayjs(period?.startDate).isAfter(dayjs())
    ) {
      return false;
    }
...
export function useLocalFilters({ students, classes, periods, filters }) {
  ...
  return React.useMemo(
    () => ({
      students: cache(
        "students",
        filterStudentsByLocalFilters({ filters, values: students })
      ),
      classes: cache(
        "classes",
        filterClassesByLocalFilters({ filters, classes })
      ),
      periods: cache(
        "periods",
        filterPeriodsByLocalFilters({ filters, periods })
      ),
    }),
