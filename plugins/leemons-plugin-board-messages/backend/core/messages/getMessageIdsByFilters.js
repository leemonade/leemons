...
async function getMessageIdsByFilters({ item, ctx }) {
  ...
  if (item.centers && item.centers.length) {
    ...
    const items = await ctx.tx.db.MessageConfigCenters.find(q)
  ...
  if (item.programs && item.programs.length) {
    ...
    const items = await ctx.tx.db.MessageConfigPrograms.find(q)
      .select(["messageConfig"])
      .lean();
    ids = _.map(items, "messageConfig");
  }
  if (item.profiles && item.profiles.length) {
    ...
    const items = await ctx.tx.db.MessageConfigProfiles.find(q)
      .select(["messageConfig"])
      .lean();
    ids = _.map(items, "messageConfig");
  }
  if (item.classes && item.classes.length) {
    ...
    const items = await ctx.tx.db.MessageConfigClasses.find(q)
      .select(["messageConfig"])
      .lean();
    ids = _.map(items, "messageConfig");
  }
