...
async function byIds({ ids: _ids, ctx }) {
  ...
  const [configs, centers, classes, profiles, programs] = await Promise.all([
    ...
    ctx.tx.db.MessageConfigCenters.find(query2)
      .select(["messageConfig", "center"])
      .lean(),
    ctx.tx.db.MessageConfigClasses.find(query2)
      .select(["messageConfig", "class"])
      .lean(),
    ctx.tx.db.MessageConfigProfiles.find(query2)
      .select(["messageConfig", "profile"])
      .lean(),
    ctx.tx.db.MessageConfigPrograms.find(query2)
      .select(["messageConfig", "program"])
      .lean(),
  ]);
  ...
  _.forEach(configs, (config, index) => {
    if (config.status === "published" && now > config.endDate) {
      ...
      updateToCompletePromises.push(
        ctx.tx.db.MessageConfig.updateOne(
          { id: config.id },
          { status: "completed" }
        )
      );
  ...
  return _.map(configs, (config) => ({
    ...
    centers: centersByConfig[config.id]
      ? _.map(centersByConfig[config.id], "center")
      : [],
    classes: classesByConfig[config.id]
      ? _.map(classesByConfig[config.id], "class")
      : [],
    profiles: profilesByConfig[config.id]
      ? _.map(profilesByConfig[config.id], "profile")
      : [],
    programs: programsByConfig[config.id]
      ? _.map(programsByConfig[config.id], "program")
      : [],
  }));
}
