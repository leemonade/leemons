async function getData({ ctx, locale }) {
    ...
    },
    {
      $project: {
        ...
        teacherLastConnection: {
          $arrayElemAt: ["$teacherLastConnection.createdAt", 0],
        },
        studentLastConnection: {
          $arrayElemAt: ["$studentLastConnection.createdAt", 0],
        },
      },
    },
  ];

  const response = await ctx.db.Class.aggregate(pipeline);
  const result = response.filter(
    (item) => item.teacherUserAgent || item.studentUserAgent
  );

  // Contatenate both student and teacher userAgent in single array
  const userAgentIds = Array.from(
    new Set(
      result.flatMap((item) => [item.teacherUserAgent, item.studentUserAgent])
    )
  ).filter(Boolean);
  ...
  // Add studentTags and teacherTags to the result
  result.forEach((item) => {
    const studentUserAgentIndex = userAgentIds.indexOf(item.studentUserAgent);
    const teacherUserAgentIndex = userAgentIds.indexOf(item.teacherUserAgent);
    const studentTags = tags[studentUserAgentIndex];
    const teacherTags = tags[teacherUserAgentIndex];
    item.studentTags = studentTags;
    item.teacherTags = teacherTags;

    delete item.studentUserAgent;
    delete item.teacherUserAgent;
  });
