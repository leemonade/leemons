async function getData({ ctx, locale }) {
        ...
        pipeline: [
          {
            $match: {
              $expr: {
                $and: [{ $eq: ['$class', '$$classId'] }, { $eq: ['$type', 'main-teacher'] }],
              },
            },
          },
          {
  ...
  const result = response.filter((item) => item.teacherUserAgent || item.studentUserAgent);

  // Contatenate both student and teacher userAgent in single array
  const userAgentIds = Array.from(
    new Set(result.flatMap((item) => [item.teacherUserAgent, item.studentUserAgent]))
  ).filter(Boolean);
