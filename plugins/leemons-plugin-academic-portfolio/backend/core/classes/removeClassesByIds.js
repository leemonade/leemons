...
const {
  removeByClass: removeKnowledgeByClass,
} = require("./knowledge/removeByClass");
const {
  removeByClass: removeSubstageByClass,
} = require("./substage/removeByClass");
const {
  removeByClass: removeStudentsByClass,
} = require("./student/removeByClass");
const {
...
async function removeClassesByIds({ ids, soft, ctx }) {
  ...
  const refClasses = await ctx.tx.db.Class.find({
