...
async function classDetailForDashboard({ classId, teacherType, ctx }) {
  ...
  const hasPermission = await ctx.tx.call(
    "users.permissions.userAgentHasCustomPermission",
    {
  ...
  const [programClasses, students, parentStudents, teachers, program] =
    await Promise.all([
      ...
      ctx.tx.db.Programs.findOne({ id: classe.program })
        .select(["hideStudentsToStudents"])
        .lean(),
    ]);
