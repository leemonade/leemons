...
async function addClass({ data, ctx }) {
  try {
    ...
    const program = await ctx.tx.db.Programs.findOne({ id: data.program })
      ...
      .lean();
    let programCenterId = await ctx.tx.db.ProgramCenter.findOne({
      program: program.id,
    }).lean();
    ...
    if (knowledgeArea) {
      ...
      if (
        !(await knowledgeAreaExistsInCenter({
          id: knowledgeArea,
        ...
        throw new LeemonsError(ctx, {
          message: "The knowledge area does not exist in Center.",
        });
      }
      promises.push(
        addKnowledge({ class: nClass.id, knowledge: knowledgeArea, ctx })
      );
    }
    if (substage) {
      ...
      if (
        ...
        throw new LeemonsError(ctx, {
          message: "The substage does not exist in Program.",
        });
      }
    ...
    */
    ...
    if (schedule) {
      promises.push(
        processScheduleForClass({ schedule, classId: nClass.id, ctx })
      );
    }
