...
const {
  removeProgramCentersByProgramIds,
} = require("./removeProgramCentersByProgramIds");
const {
  removeProgramConfigsByProgramIds,
} = require("./removeProgramConfigsByProgramIds");
...
async function removeProgramByIds({ ids, soft, ctx }) {
  ...
  await removeProgramConfigsByProgramIds({
    programIds: _.map(programs, "id"),
    soft,
    ctx,
  });
  await removeProgramCentersByProgramIds({
    programIds: _.map(programs, "id"),
    soft,
    ctx,
  });
