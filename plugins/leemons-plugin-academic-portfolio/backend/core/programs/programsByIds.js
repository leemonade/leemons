...
} = require("./getProgramCustomNomenclature");
...
async function programsByIds({
  ...
  if (onlyProgram) {
    return ctx.tx.db.Programs.find(
  ...
  let finalPrograms = programs.map((program, i) => ({
    ...
    courses: coursesByProgram[program.id]
      ? _.orderBy(coursesByProgram[program.id], "index")
      : [],
    knowledgeAreas: getUsedKnowledgeAreas(classesByProgramId[program.id] || []),
    subjects: subjectsByProgram[program.id]
      ? subjectsByProgram[program.id]
      : [],
    ...
    customSubstages: substageByProgram[program.id]
      ? _.filter(
          substageByProgram[program.id],
          ({ number }) => !_.isNil(number)
        )
      : [],
