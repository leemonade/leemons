...
function handleNonSequentialAndSingleCourses({
  groups,
  knowledgeAreas,
  subjects,
  classes,
  ...
  if (groups.length > 0) {
      ...
      .map((group) => ({
        ...
        children: subjects
          .filter((subject) =>
            classes.some(
              (cls) =>
                cls.groups?.id === group.id && cls.subject.id === subject.id
            )
          )
          .map(({ id, name }) => ({
            id,
    ...
    const groupedSubjectIds = tree.flatMap((group) =>
      group.children.map((subject) => subject.id)
    );
    const ungroupedSubjects = subjects.filter(
      (subject) => !groupedSubjectIds.includes(subject.id)
    );
    tree.push(
      ...ungroupedSubjects.map(({ id, name }) => ({
        id,
        name,
        type: "subject",
      }))
    ...
    tree = knowledgeAreas.map((knowledgeArea) => ({
      ...
      children: subjects
        .filter((subject) =>
          classes.some(
            (cls) =>
              cls.knowledges?.id === knowledgeArea.id &&
              cls.subject.id === subject.id
          )
        )
  ...
  tree.forEach((node) => {
    ...
    targetCourses.forEach((course) => {
      const courseClasses = classes.filter(
        (cls) => cls.courses?.id === course.id
      );
      const courseGroups = groups.filter(
        (group) => group.metadata.course === course.index
      );
      ...
      if (courseGroups.length > 0) {
        course.children = courseGroups
          .map((group) => {
            const groupClasses = courseClasses.filter(
              (cls) => cls.groups?.id === group.id
            );
        ...
        const subjectIds = subjects.filter((subject) =>
          subject.courses.includes(course.id)
        );
        course.children = subjectIds.map(({ id, name }) => ({
          id,
          name,
          type: "subject",
        }));
...
async function getAcademicTree({ programId, ctx }) {
  const [programDetails] = await programsByIds({
    ids: programId,
    withClasses: true,
    ctx,
  });
