...
async function getProgramEvaluationSystem({ id, ctx }) {
  ...
  if (program.evaluationSystem) {
    ...
    evaluationSystem.minScale = find(evaluationSystem.scales, {
      number: minNumber,
    });
    evaluationSystem.maxScale = find(evaluationSystem.scales, {
      number: maxNumber,
    });
  ...
  throw new LeemonsError(ctx, {
    message: "This program does not have evaluation system",
  });
}
