...
async function getUserSubjectIds({
  ...
  if (
    profile === SYS_PROFILE_NAMES.TEACHER ||
    profile === SYS_PROFILE_NAMES.STUDENT
  ) {
    const [studentClasses, teacherClasses] = await Promise.all([
      ctx.tx.db.ClassStudent.find({ student: userAgentIds })
        .select(["class"])
        .lean(),
    ...
    const classeIds = _.uniq(
      _.map(studentClasses, "class").concat(_.map(teacherClasses, "class"))
    );

    const classes = await ctx.tx.db.Class.find({ id: classeIds })
      .select(["subject"])
      .lean();
  ...
  if (profile === SYS_PROFILE_NAMES.ADMIN) {
    ...
    query.program = centerPrograms.map(
      (centerProgram) => centerProgram.program
    );
