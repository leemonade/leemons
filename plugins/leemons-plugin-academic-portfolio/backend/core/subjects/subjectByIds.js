...
} = require("../knowledges/getKnowledgeAreasBySubjects");

// Todo: Make this function subject details.
async function subjectByIds({
  ids,
  withClasses = false,
  showArchived,
  shouldPrepareAssets,
  ...
  const [subjects, creditsAndInternalId, classes, customPeriods] =
    await Promise.all([
  ...
  const subjectTypesBySubject = _.mapValues(
    classesIdsBySubject,
    (classGroup) => {
  ...
  let finalSubjects = _.map(subjects, (subject) => ({
    ...subject,
    credits:
      creditsAndInternalId.find((item) => item.subject === subject.id)
        ?.credits || null,
    internalId:
      creditsAndInternalId.find((item) => item.subject === subject.id)
        ?.internalId || null,
    ...
    customPeriod: customPeriods[subject.id]
      ? {
          startDate: customPeriods[subject.id].startDate,
          endDate: customPeriods[subject.id].endDate,
        }
      : undefined,
  }));

  if (withClasses) {
    const detailedClasses = await ctx.tx.call(
    ...
    const classesBySubject = _.groupBy(
      detailedClasses,
